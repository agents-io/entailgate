...
function usage(): string {
  return [
    "Entailgate 0.1.1 (v0.2 alpha modules available)",
    "",
    "Usage:",
    "  entailgate verify --request REQUEST.json [--trace-dir DIR]",
    "  entailgate claims extract --input DRAFT.txt",
    "  entailgate claims diff --before OLD.txt --after NEW.txt [--trace-dir DIR]",
    "  entailgate revision show PLAN.json",
    "  entailgate revision replay PLAN.json",
    "  entailgate ingest --input SOURCE [--ocr report|perform]",
    "  entailgate legal scope --draft DRAFT",
    "  entailgate legal cache-source --source FILE --cache-dir DIR --canonical-id ID",
    "             --title TITLE --issuer ISSUER --class CLASS --tier TIER",
    "             --jurisdiction JURISDICTION [--uri URI] [--ocr report|perform]",
    "  entailgate legal init --draft DRAFT --out MANIFEST.json",
    "  entailgate legal --draft DRAFT --manifest MANIFEST.json [--trace-dir DIR]",
    "  entailgate dogfood init --draft DRAFT --evidence-dir DIR --workspace DIR",
    "                   [--ocr report|perform]",
    "  entailgate dogfood status --workspace DIR",
    "  entailgate dogfood verify --workspace DIR [--trace-dir DIR]",
    "  entailgate attest --request REQUEST.json --claim-id ID --checker NAME@VERSION",
    "             --kind human|model|hybrid [--scope subject|claim]",
    "             --verdict VERDICT --score 0..1 --reason TEXT",
    "  entailgate trace show TRACE.json",
    "  entailgate replay TRACE.json",
    "",
